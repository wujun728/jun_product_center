(function () {
    ...
    // TODO EasyWeb 解决同域名多个项目
    var PATH_NAME = window.location.pathname;
    PATH_NAME = PATH_NAME.replace(/\//g, '.');
    var LOCAL_STORAGE = {
        EXPIRE_KEY: 'docsify.search.expires' + PATH_NAME,
        INDEX_KEY: 'docsify.search.index' + PATH_NAME
    };
    ...
    function genIndex(path, content, router, depth) {
        ...
        tokens.forEach(function (token) {
            if (token.type === 'heading' && token.depth <= depth) {
                // TODO EasyWeb 解决:id不能解析的问题
                var ref = getAndRemoveConfig(token.text);
                var str = ref.str;
                // slug = router.toURL(path, {id: slugify(token.text)});
                slug = router.toURL(path, {id: slugify(ref.config.id || str)});
