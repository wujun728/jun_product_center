<template>  
    <view class="container">  
		...
		<view class="user-section">
			...
			<view class="user-info-box" @click="navTo('/pages/userinfo/userinfo')">
				<view class="portrait-box">
					<image class="portrait" :src="imgMemberPath(unifiedUser.avatar)"></image>
				</view>
				<view class="info-box" v-if="unifiedUser.id>0">
					<text class="username">{{unifiedUser.nickName || unifiedUser.username}}</text>
				</view>
				<view class="info-box" v-if="unifiedUser.id<=0">
					<text class="username">{{unifiedUser.nickName || '游客'}}</text>
				</view>
			</view>
		...
		>
			...
			<view class="tj-sction" v-if="unifiedUser.account!=null">
				<view class="tj-item">
					<text class="num">{{unifiedUser.account.account}}</text>
					<text>余额</text>
				</view>
				<!-- <view class="tj-item">
				...
				<view class="tj-item">
					<text class="num">{{unifiedUser.account.score}}</text>
					<text>积分</text>
				</view>
			</view>
			<view class="tj-sction" v-if="unifiedUser.account==null">
			...
			<view class="history-section icon">
				...
				<scroll-view scroll-x class="h-list">
					<image v-for="(item, index) in footList" :key="index" @click="navTo('/pages/product/product?id='+item.referid)" :src="imgPath(item.data.img)" mode="aspectFill"></image>
        ...
        methods: {
			...
			async loadData() {
				...
				if(this.hasLogin){
					...
					//获取足迹数据
					this.sendRequest({
						url : "get_my_foot_list",
						data : {pageNo:1,pageSize:10},
						method : "get",
						success:function (res) {
							if(res.code=='200'){ 
								let footList=res.data.datas;
								footList.forEach(item=>{
									item.data=JSON.parse(item.content);
								})
								_this.footList=footList;
							} 
						}
					}); 
				}
