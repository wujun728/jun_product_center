<template>
	<view>
		...
		<navigator url="/pages/address/address?source=1" class="address-section">
			<view class="order-content" v-if="orderInfo.addressData!=null">
				<text class="yticon icon-shouhuodizhi"></text>
				<view class="cen">
					<view class="top">
						<text class="name">{{orderInfo.addressData.name}}</text>
						<text class="mobile">{{orderInfo.addressData.phone}}</text>
					</view>
					<text class="address">{{orderInfo.addressData.address}} {{orderInfo.addressData.area}}</text>
				</view>
		...
		<view class="goods-section">
			...
			<view class="g-item" v-for="(item, index) in orderInfo.goodsData" :key="item.id">
				<image :src="imgPath(item.img)"></image>
				<view class="right">
					<text class="title clamp">{{item.name}}</text>
					<text class="spec">{{item.remark}}</text>
					<view class="price-box">
		...
		<view class="yt-list">
			<view class="yt-list-cell b-b">
				...
				<text class="cell-tip">￥{{orderInfo.total}}</text>
			</view>
			<!-- <view class="yt-list-cell b-b">
			...
			<view class="yt-list-cell desc-cell">
				...
				<input class="desc" type="text" v-model="orderInfo.remark" placeholder="请填写备注信息" placeholder-class="placeholder" />
			</view>
		</view>
		...
		<view class="footer">
			<view class="price-content">
				...
				<text class="price">{{orderInfo.total}}</text>
			</view>
...
<script>
	export default {
		...
		onLoad(option){
			...
			//console.log(data);
 			if(option.orderNo){
				let _this=this;
				this.sendRequest({
					url : "get_create_order",
					data : {orderNo:option.orderNo},
					method : "get",
		...
		methods: {
			...
			submit(){
				if(!this.orderInfo.goodsData||this.orderInfo.goodsData.length<=0){
					this.$api.msg('下单失败');
					return;
				}
				if(!this.orderInfo.addressData||this.orderInfo.addressData.id<=0){
				...
				// #ifdef  MP-WEIXIN
					platform='wx';
				// #endif 
				this.orderInfo.platform=platform;//设置提交平台
				let _this=this;
				_this.sendRequest({
					url : 'save_order',
					data : this.orderInfo,
