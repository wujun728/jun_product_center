  ...
  <div class="app-container">
    ...
    <div class="operate-container" shadow="never" style="margin-top: 15px">
        <div slot="header" class="clearfix">
          <el-button size="mini" class="btn-add" @click="handleAdd" v-has="'sys:payments:add'">添加</el-button>
          <el-button size="mini" type="danger" @click="delall()" :disabled="this.multipleSelection.length===0" v-has="'sys:payments:delete'">批量删除</el-button>
        </div>
       <el-table @selection-change="handleSelectionChange" v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%;margin-top: 15px">
        ...
        <el-table-column  align="center" label="适应平台">
          <template slot-scope="scope">
        ...
        <el-table-column align="center" label="操作" width="180">
          <template slot-scope="scope">
            <el-button size="small" @click="handleEdit(scope.$index, scope.row)" v-has="'sys:payments:edit'">编辑</el-button>
					  <el-button type="danger" size="small" @click="handleDel(scope.$index, scope.row)" v-has="'sys:payments:delete'">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    <pagination v-show="total>0" :total="total" :page.sync="listQuery.pageNo" :limit.sync="listQuery.pageSize" @pagination="getList" />
        ...
        <el-form-item label="类型" prop="type">
           <el-select v-model="addForm.type" placeholder="请选择" size="mini">
              <el-option v-for="(item,index) in order_pay_typeList"
                  :key="index"
                  :label="item.name"
         ...
         <el-form-item label="平台" prop="platform">
					<el-select v-model="addForm.platform" placeholder="请选择" size="mini">
              <el-option v-for="(item,index) in payments_platformList"
                  :key="index"
                  :label="item.name"
                  :value="item.value"></el-option>
          </el-select>
    ...
    <!--编辑界面-->
        ...
        <el-form-item label="类型" prop="type">
         ...
         <el-form-item label="平台" prop="platform">
					<el-select v-model="editForm.platform" placeholder="请选择" size="mini">
              <el-option v-for="(item,index) in payments_platformList"
                  :key="index"
                  :label="item.name"
                  :value="item.value"></el-option>
...
<script>
import { fetchList,deleteEntity,removeEntity,addEntity,updateEntity} from '@/api/payments'
...
export default {
  ...
  data() {
    return {
      ...
      is_showMap:{},//是否显示
      order_pay_typeList:[],//支付方式list
      order_pay_typeMap:{},//支付方式
      payments_platformList:[],//支付平台list
      payments_platformMap:{},//支付平台
  ...
  created() {
    ...
    this.get_dictionary_list('payments_platform').then(response => {
      this.payments_platformList=response.data;
      var map = {};
      this.payments_platformList.forEach(item=>{
          map[item.value+''] = item.name;
      });
      this.payments_platformMap=map;
    })
  },
  methods: {
    //全选
