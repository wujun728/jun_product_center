    ...
    value_c() {
      ...
      switch (this.type) {
        ...
        case TYPE_LOOP:
          ...
          // console.log(`${this.valueLoop.start}-${this.valueLoop.end}/${this.valueLoop.interval}`)
          result.push(`${this.valueLoop.start}-${this.valueLoop.end}/${this.valueLoop.interval}`)
          break
        case TYPE_WORK:
      ...
      try {
        if (!value || value === this.DEFAULT_VALUE) {
          ...
          if (values.length >= 2) {
            ...
            // console.log(tvalue)
            this.valueLoop.start = tvalue[0] === '*' ? 0 : parseInt(tvalue[0])
            this.valueLoop.end = tvalue[1] === '*' ? 0 : parseInt(tvalue[1])
