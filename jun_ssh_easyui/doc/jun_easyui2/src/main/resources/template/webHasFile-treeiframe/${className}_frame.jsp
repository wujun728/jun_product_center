...
<script>
	...
	$(function() {
		sy_tabs = $("#sy_tabs");
		UploadDg = $('#i_sy_${classNameLower}_datagrid_upload_dialog');
		UploadFm =$('#i_sy_${classNameLower}_datagrid_upload_dialog_form');
		UploadFm.attr('action',"${gloabpath?replace("/","")}"+actionUrl('/sys/','${classNameLower}','Upload'));
		//加载树
		sy_tree=$("#sy_tree").tree({
			animate:true,
			url:'${gloabpath?replace("/","")}'+actionUrl('/sys/','${classNameLower}','Get_Tree'),
			onClick : function(n) {
				var sy_ep_currnode = sy_tree.tree('getSelected');
				var subtitle = sy_ep_currnode.text;
				var id = sy_ep_currnode.id;
				var sy_ep_tabs_url = "${gloabpath?replace("/","")}/sys/${className}.jsp?id="+id+"&topthemeName="+top.themeName;
				//tabs
		...
		sy_tabs.tabs('addIframeTab', {
			...
			iframe : {
				src : '${gloabpath?replace("/","")}/sys/${className}.jsp?pview=add'+"&topthemeName="+top.themeName
			}
		});
		sy_tabs.tabs('addEventParam');
	});
    ...
    <input type="text" name="fileid"  style="display:none;"/>上传文件：<input type="file" name="fileGroup"><a href="${gloabpath?replace("/","")}/excelmodel/组织表.xls" target="_blank">导入模版下载</a></br><span style="color:red">注：由于服务器的空间有限，上传文件大小不能超过1G</span></br>
