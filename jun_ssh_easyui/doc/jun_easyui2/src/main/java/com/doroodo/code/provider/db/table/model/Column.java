package com.doroodo.code.provider.db.table.model;
...
import com.doroodo.code.provider.db.table.model.ForeignKey.ReferenceKey;
import com.doroodo.code.provider.db.table.model.util.ColumnHelper;
import com.doroodo.code.util.GLogger;
import com.doroodo.code.util.StringHelper;
import com.doroodo.code.util.TestDataGenerator;
import com.doroodo.code.util.typemapping.ActionScriptDataTypesUtils;
import com.doroodo.code.util.typemapping.DatabaseDataTypesUtils;
import com.doroodo.code.util.typemapping.JavaPrimitiveTypeMapping;
...
public class Column {
	...
	private String fieldRemarks=""; //doroodo平台附加备注，表单信息
	
	private String map="";//doroodo平台附加，用于字段转换
	...
	public String getMap(){
		for(String str:fieldRemarks.split("\\^")){
			if(str.startsWith("map")){
				this.map=str.replace("map:'", "").replace("'", "");
				return this.map;
			}
		}
		return this.map;
	...
	public Column(Table table, int sqlType, String sqlTypeName,
			...
			String defaultValue,String remarks,String fieldRemarks) {
		_table = table;
		_sqlType = sqlType;
		_sqlName = sqlName;
