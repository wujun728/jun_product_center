...
var checkIsJiabanValid = function($dialog, $grid, $pjq){
	...
	if ($(':input[name="data.startTime"]').val().length <= 0) {
		$pjq.messager.alert('提示', '请假开始时间不能为空!', 'info');
		return false;
	}
	if ($(':input[name="data.startTime"]').val() == 'undefined') {
		$pjq.messager.alert('提示', '请假开始时间不能为空!', 'info');
		return false;
	}
	if ($(':input[name="data.endTime"]').val().length <= 0) {
		$pjq.messager.alert('提示', '请假结束时间不能为空!', 'info');
		return false;
	}
	if ($(':input[name="data.endTime"]').val() == 'undefined') {
		$pjq.messager.alert('提示', '请假结束时间不能为空!', 'info');
		return false;
	}
	...
	<form id="ff" method="post" class="form">
			...
			<table class="table" style="width: 100%;display: none">
				...
				<tr>
					...
					<td><select id="data.subType" name="data.subType"
						...
						data-options="required:true,editable:false,idField:'id',textField:'text',parentField:'pid',url:'<%=contextPath%>/base/sys-def!doNotNeedSecurity_getAllListByType.sy?type=QINGJIA_TYPE'"
						style="width: 155px;"></select></td>
					<th><!-- 类型 --></th>
					<td>
					<input class="easyui-validatebox" hidden="false"
						type="text" name="data.type" value="2"
						data-options="required:false"></input>
					<%-- <select class="easyui-combobox" name="data.type" 
						data-options="panelHeight:'auto',editable:false"  value="<%=opType %>"
						data-options="required:true" style="width: 155px;">
							<option value="1"  <%if(opType.equals("1")) { %> selected="selected" <%} %>>加班</option>
							<option value="2"  selected="selected"  >请假</option>
					</select> --%>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<th>备注</th>
					<td><textarea name="data.ext1" cols="100"></textarea></td>
				</tr>
				<tr>
					<th>退回原因</th>
		...
		<div style="text-align: left; padding: 5px">
								<input id="iconCls" class="ext-icon-bell" readonly="readonly"
									style="padding-left: 18px; width: 13px; border: none" />注意：请假支持一段时间，请假的日期系统自动获取当天日期
		</div> 
	</form>
</body>
</html>