...
<script type="text/javascript">
	...
	var addFun = function() {
		var dialog = parent.sy.modalDialog({
			title : '添加自定义信息',
			url : sy.contextPath + '/securityJsp/app/SelfMedicalDefForm.jsp',
			buttons : [ {
				text : '添加',
				handler : function() {
	...
	var showFun = function(id) {
		var dialog = parent.sy.modalDialog({
			title : '查看自定义信息',
			url : sy.contextPath + '/securityJsp/app/SelfMedicalDefForm.jsp?id=' + id
		});
	};
	...
	var removeFun = function(id) {
		parent.$.messager.confirm('询问', '您确定要删除此记录？' +id, function(r) {
			if (r) {
				$.post(sy.contextPath + '/app/self-medical-def!delete.sy', {
					selfMedicalDefId : id
				}, function() {
		...
		grid = $('#grid').datagrid({
			...
			columns : [ [  {
				...
				formatter : function(value, row) {
					var str = '';
					<%if (securityUtil.havePermission("/app/self-medical-def!getById")) {%>
						var bt = systool.createFuncButton('ext-icon-note','查看','查看自定义资料','showFun(\'{0}\');');
						str +=  sy.formatString(bt,row.defId);
					<%}%>
					<%if (securityUtil.havePermission("/app/self-medical-def!update")) {%>
						var bt = systool.createFuncButton('ext-icon-note_edit','编辑','编辑自定义资料','editFun(\'{0}\');');
						str +=  sy.formatString(bt,row.defId);
					<%}%>
					<%if (securityUtil.havePermission("/app/self-medical-def!delete")) {%>
