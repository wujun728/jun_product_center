...
<script type="text/javascript">
	...
	var addFun = function() {
		var dialog = parent.sy.modalDialog({
			title : '添加体检信息',
			url : sy.contextPath + '/securityJsp/app/MedicalDetailForm.jsp?oType=TJ_REPORT_TYPE_YEAR',
			buttons : [ {
				text : '添加',
				handler : function() {
	...
	var addFunRe = function(id) {
		var dialog = parent.sy.modalDialog({
			title : '添加体检信息',
			url : sy.contextPath + '/securityJsp/app/MedicalDetailForm.jsp?oType=TJ_REPORT_TYPE_YEAR&data_empMedicalDetail_id=' + id ,
			buttons : [ {
				text : '添加',
				handler : function() {
					dialog.find('iframe').get(0).contentWindow.submitForm(
	...
	var showFun = function(id) {
		var dialog = parent.sy.modalDialog({
			title : '查看体检信息',
			url : sy.contextPath + '/securityJsp/app/MedicalDetailForm.jsp?oType=TJ_REPORT_TYPE_YEAR&id=' + id
		});
	};
	...
	var editFun = function(id) {
		var dialog = parent.sy
				.modalDialog({
					title : '编辑体检信息',
					url : sy.contextPath
							+ '/securityJsp/app/MedicalDetailForm.jsp??oType=TJ_REPORT_TYPE_YEAR&id=' + id,
					buttons : [ {
						text : '编辑',
						handler : function() {
	...
	$(function() {
		...
		medicalTypeCombox= $('#medicalTypeCombox').combobox({
	        url:'<%=contextPath%>/base/sys-def!doNotNeedSecurity_getSubListByType.sy?type=TJ_REPORT_TYPE_YEAR',
	        idField:'id',
	        valueField:'id',
	        textField:'text',
	        parentField:'pid',
		...
		grid = $('#grid').treegrid(
				{
					title : '',
					url : sy.contextPath + '/app/medical-detail!grid.sy?data.medicalType=10104101',
					striped : false,
					rownumbers : true,
					pagination : true,
					singleSelect : true,
					...
					<form id="searchForm">
						<table>
							<tr>
								...
								<td>体检性质</td>
								<td>
									<select id="medicalTypeCombox" name="QUERY_t#medicalType_I_EQ" class="easyui-combotree"  style="width: 155px;" ></select>
						...
						<table>
							<tr>
								...
								<td>
									...
									<select id="attrUnitCombox" name="QUERY_t#attrUnit_S_EQ" class="easyui-combotree"  style="width: 105px;"></select><img class="iconImg ext-icon-cross" onclick="$('#attrUnitCombox').combotree('clear');" title="清空" />
								</td>
											<th>体检科室</th>
											<td>
