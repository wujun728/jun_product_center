...
<script type="text/javascript">
	...
	$(function() {
		...
		grid = $('#grid').datagrid({
			title : '',
			url : sy.contextPath + '/app/cust-user!grid.sy?custType=0',
			striped : true,
			rownumbers : true,
			pagination : true,
			singleSelect : true,
			...
			columns : [ [ {
				...
				formatter : function(value, row) {
					...
					<%-- <%if (securityUtil.havePermission("/app/cust-user!grantCustDept")) {%>
						var bt = systool.createFuncButton('ext-icon-group','分配部门','给员工分配部门','grantOrganizationFun(\'{0}\');');
						str +=  sy.formatString(bt,row.userId);
					<%}%> --%>						
					...
					<%if (securityUtil.havePermission("/app/cust-user!gotoEmpIllHis")) {%>
						var bt = systool.createFuncButton('ext-icon-user','员工病史','员工病史管理','gotoEmpIllHis(\'{0}\',\'{1}\');');
						str +=  sy.formatString(bt,row.userId,row.userName);
					<%}%>
					<%if (securityUtil.havePermission("/app/cust-user!gotoEmpHis")) {%>
						var bt = systool.createFuncButton('ext-icon-user','员工职业史','员工职业史管理','gotoEmpHis(\'{0}\',\'{1}\');');
						str +=  sy.formatString(bt,row.userId,row.userName);
					<%}%>
					return str;
				}
