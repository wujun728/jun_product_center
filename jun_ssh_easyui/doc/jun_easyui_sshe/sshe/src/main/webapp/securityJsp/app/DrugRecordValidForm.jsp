	...
	var submitForm  = function($op,$dialog, $grid, $pjq) {
		if ($('form').form('validate')) 
		{
			...
			if ($(':input[name="data.drugSpecInfo.specId"]').val() > 0) {
				if($op == 'rk'){
					url = sy.contextPath + '/app/drug-in-record!valid.sy?opMod=IN';
				}else if($op == 'pd'){
					url = sy.contextPath + '/app/drug-in-record!valid.sy?opMod=PANDIAN';
				}else if($op == 'JZ'){
					url = sy.contextPath + '/app/drug-in-record!valid.sy?opMod=JIUZHG';
				}
					...
					if (result.obj != null){
						...
						if (result.obj.id != undefined) {
									$('form').form('load',{
														...
														'data.drugInfo.drugCode' : result.obj.drugSpecInfo.drugInfo.drugCode,
														'data.num' : result.obj.num,
														'data.price' : result.obj.price,
														'data.drugInfo.specification' : result.obj.drugSpecInfo.drugInfo.specification
	...
	<div class="easyui-panel" title=""  style="height: 526px" data-options="fit:true,border:false" >
		<div style="padding: 10px 0 10px 10px;"   >
								...
								<form id="form" method="post">
									<table>
										...
										<tr>
											...
											<th>规格</th>
											<td><input name="data.specification"  
										...
										<tr>
											<th>盘点数量</th>
											<td><input name="data.num2" class="easyui-numberbox" readonly="readonly"
			...
			<div style="text-align: left; padding: 5px">
				<input id="iconCls"  class="ext-icon-bell" readonly="readonly" style="padding-left: 18px; width: 13px;border: none" />盘点纠正：把库存的数量改为盘点的数量
			</div>
		</div>
	</div>
