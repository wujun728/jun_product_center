...
public class SysFilesServiceImpl extends ServiceImpl<SysFilesMapper, SysFilesV2Entity> implements SysFilesService {
    @Resource
	...
	private void saveFilesEntity(String fileName, String newFilePathName, String url, String biztype, String bizid) {
		SysFilesV2Entity entity = new SysFilesV2Entity();
		entity.setFileName(fileName);
		entity.setFilePath(newFilePathName);
		entity.setUrl(url);
		entity.setDictBiztype(String.valueOf(mapTmp.get().get("biztype")));
    ...
    public void removeByIdsAndFiles(List<String> ids) {
        List<SysFilesV2Entity> list = this.listByIds(ids);
        list.forEach(entity -> {
            //如果之前的文件存在，删除
