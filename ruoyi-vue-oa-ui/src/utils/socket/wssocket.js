...
let connect = (wsurl, accessToken, terminal) => {
	try {
		if (isConnect) {
		...
		websock = new WebSocket(wsurl + "?Authorization=" + accessToken + "&terminal=" + terminal);
		...
		websock.onerror = function (err) {
			...
			reconnect(wsurl, accessToken, terminal);
		}
	} catch (e) {
		console.log("尝试创建连接失败", e);
		reconnect(wsurl, accessToken, terminal); //如果无法连接上webSocket 那么重新连接！可能会因为服务器重新部署，或者短暂断网等导致无法创建连接
	}
};

//定义重连函数
let reconnect = (wsurl, accessToken, terminal) => {
	console.log("尝试重新连接");
	if (isConnect) {
		//如果已经连上就不再重连了
		return;
	...
	rec = setTimeout(function () { // 延迟5秒重连  避免过多次过频繁请求重连
		connect(wsurl, accessToken, terminal);
	}, 15000);
};
//设置关闭连接
let close = (code) => {
