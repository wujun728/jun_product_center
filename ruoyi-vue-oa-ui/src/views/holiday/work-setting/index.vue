<template>
  <div class="tab">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="92x">
    ...
    <el-table v-loading="loading" :data="settingList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="关联节假日" align="left" prop="holidayName" width="200" />
      <el-table-column label="补班日期" align="center" prop="workDate" width="160">
    ...
    <el-dialog :title="title" :visible.sync="open" width="800px" append-to-body>
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="92px">
        ...
        <el-form-item label="补班日期" prop="workDate">
          <el-date-picker clearable v-model="form.workDate" type="date" value-format="yyyy-MM-dd" placeholder="请选择补班日期"></el-date-picker>
        </el-form-item>
        <el-form-item label="关联节假日" prop="holidaySettingId">
          <el-select v-model="form.holidaySettingId">
            <el-option v-for="item in holidayList" :key="item.value" :value="item.value" :label="item.label" />
          </el-select>
        </el-form-item>
...
<script>
import { listSetting, getSetting, delSetting, addSetting, updateSetting } from "@/api/system/holidayWorkSetting";
import { listYearHoliday } from "@/api/system/holidaySetting";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        workDate: [{ required: true, message: "补班日期不能为空", trigger: "blur" }],
      },
      yearList: [],
      holidayList: [],
    };
  ...
  methods: {
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加节假日补班设置";
      this.getHolidayList(new Date().getFullYear());
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      ...
      getSetting(id).then((response) => {
        ...
        this.open = true;
        this.title = "修改节假日补班设置";
        this.getHolidayList(this.form.year);
      });
    },
    /** 获取节假日 */
    getHolidayList(year) {
      this.holidayList = [];
      listYearHoliday(year).then((res) => {
        if (res.data && res.data.length > 0) {
          res.data.forEach((holiday) => {
            this.holidayList.push({ label: holiday.holidayName, value: holiday.id });
          });
        }
      });
    },
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm('是否确认删除节假日补班设置编号为"' + ids + '"的数据项？')
        .then(function () {
