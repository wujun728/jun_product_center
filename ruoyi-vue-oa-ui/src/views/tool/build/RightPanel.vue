    ...
    <div class="field-box">
      ...
      <el-scrollbar class="right-scrollbar">
        ...
        <el-form v-show="currentTab==='field' && showField" size="small" label-width="90px">
          ...
          <el-form-item v-if="activeData.__vModel__!==undefined" prop="__vModel__">
            <template slot="label">
          ...
          <el-form-item v-if="activeData.tooltip!==undefined">
            <span slot="label">
              <el-tooltip content="1、如需字段提示，请填写内容并确保不为空；2、如需换行显示，请在换行位置按回车健即可。" placement="top">
                <i class="el-icon-question"></i>
              </el-tooltip>字段提示
            </span>
            <el-input v-model="activeData.tooltip" type="textarea" placeholder="请输入提示内容" @input="changeRenderKey" />
          </el-form-item>
          <el-form-item v-if="activeData.__config__.span!==undefined" label="表单栅格">
            ...
            <el-select v-model="activeData.__config__.span" placeholder="请选择排列方式" :style="{width: '100%'}" @change="spanChange">
              <el-option v-for="(item, index) in spanOptions" :key="index" :label="item.label" :value="item.value" />
            </el-select>
          </el-form-item>
          ...
          <el-form-item
            v-if="activeData.__vModel__!==undefined && !['design-user-select', 'design-dept-select'].includes(activeData.__config__.tag)"
            label="默认值"
          >
          ...
          <template v-if="['design-dept-select'].includes(activeData.__config__.tag)">
            <el-form-item label="默认值">
              <el-select v-model="activeData.initDept" placeholder="请选择默认值" :style="{width: '100%'}" @change="initDeptChange">
                <el-option v-for="(item, index) in deptSelectOptions" :key="index" :label="item.label" :value="item.value" />
              </el-select>
            </el-form-item>
          </template>

          <el-form-item v-if="['design-user-select', 'design-dept-select'].includes(activeData.__config__.tag)" label="是否多选">
...
export default {
  ...
  data() {
    return {
      ...
      userSelectOptions: [
        { label: "无", value: "none" },
        { label: "当前登录人", value: "user" },
      ],
      deptSelectOptions: [
        { label: "无", value: "none" },
        { label: "当前登录人所在部门", value: "dept" },
      ],
    };
  },
  computed: {
  ...
  methods: {
    ...
    initUserChange(val) {
      this.$set(this.activeData.__config__, "initUser", val);
    },
    initDeptChange(val) {
      this.$set(this.activeData.__config__, "initDept", val);
    },
    dateTypeChange(val) {
    ...
    handlePropInput(value) {
      this.activeData.__vModel__ = value.replace(/[^a-zA-Z0-9]/g, "");
    },
  },
};
</script>
