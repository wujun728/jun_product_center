...
<script type="text/javascript">
	...
	var editFun = function(id) {
		var dialog = parent.sy.modalDialog({
			title : '纠正药品信息',
			url : sy.contextPath + '/securityJsp/app/DrugRecordValidForm.jsp?id=' + id,
			buttons : [ {
				text : '纠正',
				handler : function() {
					dialog.find('iframe').get(0).contentWindow.submitForm('JZ',dialog, grid, parent.$);
				}
			} ]
		});
	};
		...
		grid = $('#grid').datagrid({
			...
			frozenColumns : [ [{
				...
				formatter : function(value, row, index) {
					if(value.customerInfo != undefined){
						 return value.drugName  + "[客户:"+value.customerInfo.customerName+"]";
					}else{
			...
			columns : [ [  {
				...
				formatter : function(value, row) {
					...
					<%}%> --%>
					<%if (securityUtil.havePermission("/app/drug-store!getById")) {%>
					var bt = systool.createFuncButton('ext-icon-note_delete','纠正','纠正误差','editFun(\'{0}\');');
