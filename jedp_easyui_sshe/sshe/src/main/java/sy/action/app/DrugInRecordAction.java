...
@Namespace("/app")
@Action
public class DrugInRecordAction extends BaseAction<DrugInRecord> {
	private static final Logger logger = Logger
			.getLogger(DrugInRecordAction.class);
	...
	public void grid() {
		...
		// }
		 super.addCutomerFilter(hqlFilter,"QUERY_t#customerInfo#customerId_I_EQ");
	...
	public void save()    {
		...
		if (data != null) {
			SessionInfo sessionInfo = (SessionInfo) getSession().getAttribute(
					ConfigUtil.getSessionInfoName());
			DrugSpecInfo drugSpecInfo = drugSpecInfoServiceI.getById(data.getDrugSpecInfo().getSpecId());
			...
			if (!StringUtils.isBlank(getOpMod())) {
				if (getOpMod().equalsIgnoreCase(OP_TYPE_SELF)) {
					...
					if (num < 0) {
						...
						drugRecordLog.setCustomerInfo(drugSpecInfo.getDrugInfo().getCustomerInfo());
			...
			data.setCustomerInfo(drugSpecInfo.getDrugInfo().getCustomerInfo());
