...
sys.showUserDataFun = function(id,name) {
	...
	var dialog = parent.sy.modalDialog({
		title : '查看属性信息',
		url : sy.contextPath + '/securityJsp/app/MedicalReportAttrDefForm.jsp?id=' + id + "&name=" + name
	});
};
...
sys.removeUserDataFun  = function(id) {
	parent.$.messager.confirm('询问', '您确定要删除此记录？', function(r) {
		if (r) {
			$.post(sy.contextPath + '/app/report-attr!delete.sy', {
...
sys.loadRoleUsersFun = function(id) {
	...
	var userGrid = $('#userGrid').treegrid({
		title : '',
		url : sy.contextPath + '/app/report-attr!treeGrid.sy?templateId='+id,
		striped : false,
		rownumbers : true,
		pagination : true,
		singleSelect : false,
...
sys.getSelected = function(component) {
	...
	if (row) {
		//$.messager.alert('Info', row.templateName + ":" + row.templateId) ;
	     
		sys.loadUserData(row.templateId);
