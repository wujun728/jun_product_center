			...
			},
			{
				...
				formatter : function(value, row) {
					...
					<%if (securityUtil.havePermission("/app/cust-dept!getById")) {%>
						var bt = systool.createFuncButton('ext-icon-note','查看','查看部门资料','showFun(\'{0}\');');
						str +=  sy.formatString(bt,row.deptId);
					<%}%>
					<%if (securityUtil.havePermission("/app/cust-dept!update")) {%>
					var bt = systool.createFuncButton('ext-icon-note_edit','编辑','编辑部门资料','editFun(\'{0}\');');
					str +=  sy.formatString(bt,row.deptId);	
					<%}%>
					<%if (securityUtil.havePermission("/app/cust-dept!grant")) {%>
						var bt = systool.createFuncButton('ext-icon-group_key','授权','授权部门','grantFun(\'{0}\');');
						str +=  sy.formatString(bt,row.deptId);	
					<%}%>
					<%if (securityUtil.havePermission("/app/cust-dept!delete")) {%>
					var bt = systool.createFuncButton('ext-icon-note_delete','删除','删除部门','removeFun(\'{0}\');');
					str +=  sy.formatString(bt,row.deptId);		
					<%}%>
					return str;
				}
