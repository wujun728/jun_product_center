  ...
  <div class="app-container">
    ...
    <el-form :model="queryParams" class="queryFormClass"  ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="请假类型" prop="type">
        <el-select v-model="queryParams.type" placeholder="请选择请假类型" clearable>
          <el-option v-for="dict in dict.type.bpm_oa_leave_type" :key="parseInt(dict.value)" :label="dict.label" :value="parseInt(dict.value)"/>
        </el-select>
      </el-form-item>
      ...
      <el-form-item label="结果" prop="result">
        <el-select v-model="queryParams.result" placeholder="请选择流结果" clearable>
          <el-option v-for="dict in dict.type.bpm_process_instance_result" :key="parseInt(dict.value)" :label="dict.label" :value="parseInt(dict.value)"/>
        </el-select>
      </el-form-item>
    ...
    <el-table stripe border v-loading="loading" class="tableClass"   :data="list">
      ...
      <el-table-column label="请假类型" align="center" prop="type">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.bpm_oa_leave_type" :value="scope.row.type"/>
        </template>
      </el-table-column>
...
export default {
  ...
  dicts: ['bpm_process_instance_result','bpm_oa_leave_type'],
  data() {
    return {
      // 遮罩层
      loading: true,
