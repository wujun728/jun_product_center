<template>
  <div class="app-container">
    <!-- 搜索工作栏 -->
    <el-form :model="queryParams" class="queryFormClass"  ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="流程分类" prop="category">
        <el-select v-model="queryParams.category" placeholder="流程分类" clearable style="width: 240px">
          <el-option v-for="dict in dict.type.bpm_model_category" :key="parseInt(dict.value)" :label="dict.label" :value="parseInt(dict.value)"/>
        </el-select>
      </el-form-item>
      <el-form-item>
    ...
    <el-table stripe border v-loading="loading" class="tableClass"   :data="list">
    ...
    <!-- 流程表单配置详情 -->
    <el-dialog v-drag-dialog="true" title="表单详情" :visible.sync="detailOpen" width="50%" append-to-body>
      <parser :key="new Date().getTime()" :form-conf="detailForm" />
    </el-dialog>

    <!-- 流程模型图的预览 -->
    <el-dialog v-drag-dialog="true" title="流程图" :visible.sync="showBpmnOpen" width="80%" append-to-body>
        ...
        <div v-if="form.id">
          <el-form-item label="表单类型" prop="formType">
            <el-radio-group v-model="form.formType">
              <el-radio v-for="dict in dict.type.bpm_model_form_type" :key="parseInt(dict.value)" :label="parseInt(dict.value)">
                {{dict.label}}
              </el-radio>
            </el-radio-group>
          </el-form-item>
    ...
    <!-- 用户导入对话框 -->
    <el-dialog v-drag-dialog="true" title="导入流程" :visible.sync="upload.open" width="400px" append-to-body>
...
export default {
  ...
  dicts: ['bpm_model_category','bpm_model_form_type','bpm_task_assign_rule_type'],
  data() {
    return {
      // 遮罩层
      loading: true,
