<template>
  <div>
    <!-- 列表弹窗 -->
    <el-dialog v-drag-dialog="true" title="任务分配规则" :visible.sync="visible" width="800px" append-to-body>
    ...
    <el-table stripe border v-loading="loading" class="tableClass"   :data="list">
        ...
        <el-table-column label="规则范围" align="center" prop="options" width="440px">
          <template slot-scope="scope" v-if="scope.row.options">
            <el-tag size="medium" :key="option" v-for="option in scope.row.options">
              {{ getAssignRuleOptionName(scope.row.type, option) }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column v-if="modelId" label="操作" fixed="right"  align="center" width="80" >
          <template slot-scope="scope">
    ...
    <!-- 添加/修改弹窗 -->
    <el-dialog v-drag-dialog="true" title="修改任务规则" :visible.sync="open" width="500px" append-to-body>
...
export default {
  ...
  dicts: ['bpm_model_form_type','bpm_task_assign_rule_type','bpm_task_assign_script'],
  data() {
    return {
      // 如下参数，可传递
  ...
  created() {
    this.getDicts("bpm_task_assign_script").then(response => {
      this.taskAssignScriptDictDatas = response.data;
    });
  },
  methods: {
    initModel(modelId) {
    ...
    init0() {
      ...
      listSimpleDepts().then(response => {
        ...
        this.deptTreeOptions.push(...this.handleTree(response.data, "deptId"));
      });
      // 获得岗位列表
      this.postOptions = [];
