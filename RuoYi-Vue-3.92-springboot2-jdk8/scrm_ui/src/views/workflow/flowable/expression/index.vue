    ...
    <el-table v-loading="loading" :data="expressionList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="类型" align="center" prop="dataType">
        <template slot-scope="scope">
          <dict-tag :options="dict.type.flowable_expression_type" :value="scope.row.dataType" />
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="800px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="类型" prop="type">
          <el-select v-model="form.dataType" placeholder="请选择类型" clearable style="width: 240px">
            <el-option v-for="dict in dict.type.flowable_expression_type" :key="dict.value" :label="dict.label" :value="dict.value" />
          </el-select>
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        expression: [{ required: true, message: "内容不能为空", trigger: "blur" }],
      },
    };
  },
  created() {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm('是否确认删除流程表达式名称为"' + row.name + '"的数据项？')
        .then(function () {
