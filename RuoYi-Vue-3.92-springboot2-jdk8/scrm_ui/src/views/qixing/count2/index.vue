    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:count2:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:count2:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="count2List" @selection-change="handleSelectionChange">
...
<script>
import { listCount2, getCount2, delCount2, addCount2, updateCount2 } from "@/api/qixing/count2";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listCount2(this.queryParams).then(response => {
        this.count2List = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateCount2(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addCount2(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/count2/export', {
        ...this.queryParams
      }, `count2_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
