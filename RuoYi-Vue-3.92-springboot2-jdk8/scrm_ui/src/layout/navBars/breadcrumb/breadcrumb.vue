  ...
  <div class="layout-navbars-breadcrumb">
    ...
    <el-breadcrumb class="layout-navbars-breadcrumb-hide" v-if="isShowcrumb" :style="{ display: isShowBreadcrumb }">
      <transition-group name="breadcrumb" mode="out-in">
        <el-breadcrumb-item v-for="(v, k) in [...breadCrumbList, ...crumbPast]" :key="v.id + '_' + k">
          <span v-if="k == 1" class="layout-navbars-breadcrumb-span">
            <svg-icon
              :icon-class="v.icon"
              class-name="layout-navbars-breadcrumb-iconfont"
      ...
      this.breadcrumbList = [
        {
          path: '/',
          meta: {
            title: (menuList && menuList.length > 0 && menuList[0]) ? menuList[0].title : '主页',
            icon: (menuList && menuList.length > 0 && menuList[0]) ? menuList[0].icon : 'dashboard',
          },
        },
      ];
