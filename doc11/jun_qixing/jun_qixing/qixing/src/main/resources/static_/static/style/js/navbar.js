layui.define(['element', 'common'], function(exports) {
	...
	Navbar.prototype.render = function(tab) {
		var _that = this;
		...
		if(_config.data !== undefined && typeof(_config.data) === 'object') {
			...
			if(_con.attr('lay-filter') !== undefined) {
				...
				if(flag==false){
					var $this = _con.children('ul').find('li').eq(0);
					if($this.find('dl').length > 0) {
						var $a = $this.find('dd').eq(0).children('a')
						var href = $this.find('dd').eq(0).children('a').data('url');
						var icon = $a.children('i:first').data('icon');
						var title = $a.children('cite').text();
						var data = {
							elem: $a,
						...
						$this.find('dd').eq(0).addClass("layui-this");
						$this.addClass("layui-nav-itemed");
						tab.tabAdd(data.field);
					} else {
						var $a = $this.children('a');
			...
			if(data[i].children !== undefined && data[i].children!=null && data[i].children.length > 0) {
				...
				for(var j = 0; j < data[i].children.length; j++) {
					...
					if(data[i].children[j].icon !== undefined && data[i].children[j].icon !== '') {
						...
						}else if(data[i].children[j].icon.indexOf('layui-icon') !== -1){
	...
	function getHtml(data) {
		...
		for(var i = 0; i < data.length; i++) {
			...
			// if(data[i].spread && data[i].title=='项目管理') {
			// 	ulHtml += '<li class="layui-nav-item layui-nav-itemed">';
			// } else {
			// 	ulHtml += '<li class="layui-nav-item">';
			// }
			if(data[i].children !== undefined && data[i].children!=null && data[i].children.length > 0) {
				ulHtml += '<a href="javascript:;">';
				if(data[i].icon && data[i].icon !== undefined && data[i].icon !== '') {
					if(data[i].icon.indexOf('fa-') !== -1) {
						ulHtml += '<i class="fa ' + data[i].icon + '" aria-hidden="true" data-icon="' + data[i].icon + '"></i>';
					}else if(data[i].icon.indexOf('layui-icon') !== -1){
						ulHtml += '<i class="layui-icon '+data[i].icon+'" data-icon="' + data[i].icon + '"></i>';
				...
				ulHtml += '<dl class="layui-nav-child">';
				for(var j = 0; j < data[i].children.length; j++) {
					// ulHtml += '<dd class="'+(data[i].children[j].title=='项目管理'?'layui-this':'')+'">';
					ulHtml += '<dd class="">';
					ulHtml += '<a href="javascript:;" data-url="' + data[i].children[j].href + '">';
					if(data[i].children[j].icon && data[i].children[j].icon !== undefined && data[i].children[j].icon !== '') {
						if(data[i].children[j].icon.indexOf('fa-') !== -1) {
							ulHtml += '<i class="fa ' + data[i].children[j].icon + '" data-icon="' + data[i].children[j].icon + '" aria-hidden="true"></i>';
						}else if(data[i].children[j].icon.indexOf('layui-icon') !== -1){
							ulHtml += '<i class="layui-icon '+data[i].children[j].icon+'" data-icon="' + data[i].children[j].icon + '"></i>';
