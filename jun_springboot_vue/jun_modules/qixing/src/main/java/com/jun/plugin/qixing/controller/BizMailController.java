...
public class BizMailController {
    ...
    */
    ...
    @ResponseBody
    public DataResult add(@RequestBody BizMailEntity bizMail){
    ...
    @ResponseBody
    public DataResult update(@RequestBody BizMailEntity bizMail){
    ...
    @ResponseBody
    @DataScope
    public DataResult findListByPage(@RequestBody BizMailEntity bizMail){
        Page page = new Page(bizMail.getPage(), bizMail.getLimit());
        LambdaQueryWrapper<BizMailEntity> queryWrapper = Wrappers.lambdaQuery();
        //查询条件示例
        //queryWrapper.eq(BizMailEntity::getId, bizMail.getId());
        IPage<BizMailEntity> iPage = bizMailService.page(page, queryWrapper);
    ...
    @ResponseBody
    public DataResult findOne(@RequestBody BizMailEntity bizMail){
    	LambdaQueryWrapper<BizMailEntity> queryWrapper = Wrappers.lambdaQuery();
    	//查询条件示例
    	//queryWrapper.eq(BizMailEntity::getId, bizMail.getId());
    	//BizMailEntity one = bizMailService.getOne(queryWrapper);
    	BizMailEntity one = bizMailService.getById(bizMail.getId());
    ...
    @ResponseBody
    public DataResult findListBySelect(@RequestBody BizMailEntity bizMail){
        Page page = new Page(bizMail.getPage(), bizMail.getLimit());
        LambdaQueryWrapper<BizMailEntity> queryWrapper = Wrappers.lambdaQuery();
        //查询条件示例
        //queryWrapper.eq(BizMailEntity::getId, bizMail.getId());
