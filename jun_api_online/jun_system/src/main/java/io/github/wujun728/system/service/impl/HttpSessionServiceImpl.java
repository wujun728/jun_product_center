 ...
 * @date 2020年3月18日
 */
@Service(value = "httpSessionService")
public class HttpSessionServiceImpl implements HttpSessionService {
    ...
    @Override
    public String createTokenAndUser(BaseEntity user, List<String> roles, Set<String> permissions, String jwtToken) {
        return this.createTokenAndUser((SysUser) user, roles, permissions, jwtToken);
    }
...
//    @Cacheable(cacheNames = "cache1m")
    ...
    public void abortAllUserByToken() {
        String token = getTokenFromHeader();
        String userId = HttpSessionService.getUserIdByToken(token);
