<template>
  <div class="panel-tab__content">
    <el-form label-width="82px" @submit.native.prevent size="small">
      <el-form-item label="参数说明">
        <el-button size="mini" type="primary" plain @click="dialogVisible = true">查看</el-button>
      </el-form-item>
      ...
      <template v-if="loopCharacteristics === 'ParallelMultiInstance' || loopCharacteristics === 'SequentialMultiInstance'">
        ...
        <el-form-item label key="completionCondition">
          <span slot="label">
            <el-tooltip content="只有达成此条件时，多人同时审批的环节，才会触发到下一个流程环节" placement="top">
              <i class="el-icon-question"></i>
            </el-tooltip>完成条件
          </span>
          <!-- <el-input v-model="loopInstanceForm.completionCondition" clearable @change="updateLoopCondition" /> -->
          <el-select v-model="loopInstanceForm.completionCondition" placeholder="请选择" @change="updateLoopCondition">
            <el-option v-for="item in completionConditionOpts" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
  ...
  data() {
    return {
      ...
      defaultLoopInstanceForm: {
        ...
        asyncBefore: false,
        exclusive: false,
        collection: this.id + "_approval",
        elementVariable: "assignee",
      },
      // 完成条件选择
      completionConditionOpts: [
        { label: "全部完成时触发", value: "${nrOfCompletedInstances/nrOfInstances >= 1 }" },
        { label: "完成一半时触发", value: "${nrOfCompletedInstances/nrOfInstances >= 0.5 }" },
      ],
    };
  },
